...
export const AppProvider: React.FC<{
  ...
  const [activeProduct, setActiveProduct] = useState<ProductTypeExt | null>(null);
  ...
  // UseEffect for loading data for homepage, cart and favourites
  useEffect(() => {
    setIsLoading(true);
  ...
  // UseEffect to save cart items in local storage
  useEffect(() => {
    window.localStorage.setItem('cart', JSON.stringify(cart));
  }, [cart]);

  // UseEffect to save favourites items in local storage
  useEffect(() => {
    window.localStorage.setItem('favourites', JSON.stringify(favourites));
  }, [favourites]);

  // UseEffect for handling searchParams
  useEffect(() => {
  ...
  const handleActiveProduct = useCallback(async (productId: string | null) => {
    ...
    try {
      if (productId === null) {
        ...
        if (target) {
          const product = await getProduct(target.category as CategoriesType, productId);

          setActiveProduct(product);

          const phonesFromServer = await getProducts(target.category as CategoriesType);

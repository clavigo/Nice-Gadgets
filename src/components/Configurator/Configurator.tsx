...
const productIdModifier = (namespaceId: string, capacity: string, color: string) => {
...
export const Configurator = () => {
  ...
  const { activeProduct, products, handleFavourites, handleCart, favourites, cart } =
  ...
  useEffect(() => {
    if (favourites?.some((favouriteItem) => product && favouriteItem.itemId === product.itemId)) {
      setIsLiked(true);
    }

    if (cart?.some((favouriteItem) => product && favouriteItem.product.itemId === product.itemId)) {
      setIsAddedToCart(true);
    }
  }, [favourites, cart, product]);

  //Function to handle color and capacity changes
  const handleClick = (capacity = activeProduct?.capacity, color = activeProduct?.color) => {
      ...
      <div className={styles.configurator__bottomContainer}>
        <div className={styles.configurator__priceAndButtons}>
          <div className={styles.configurator__priceContainer}>
            <h1 className={styles.configurator__actualPrice}>${activeProduct?.priceDiscount}</h1>

            <p className={styles.configurator__oldPrice}>${activeProduct?.priceRegular}</p>
          </div>

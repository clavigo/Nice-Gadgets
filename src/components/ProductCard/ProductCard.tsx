...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const { handleFavourites, favourites, handleCart, cart } = useContext(AppContext) || {};
  ...
  // UseEffect to display correct buttons state
  useEffect(() => {
    if (favourites?.some((favouriteItem) => favouriteItem.itemId === product.itemId)) {
      setIsLiked(true);
    }

    if (cart?.some((favouriteItem) => favouriteItem.product.itemId === product.itemId)) {
  ...
  return (
    <div className={styles.card}>
      <Link to={handleLink()} className={styles.card__imageContainer}>
        <img className={styles.card__image} src={`${baseUrl}/${product.image}`} alt="" />
      </Link>

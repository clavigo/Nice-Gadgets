...
export const Pagination = () => {
  const { products, perPage, currentPage, sortBy, isLoading } = useContext(AppContext)!;
  const [visibleProducts, setVisibleProducts] = useState<ProductType[]>([]);
  const [showNoProducts, setShowNoProducts] = useState(false);

  useEffect(() => {
    setVisibleProducts(getVisibleProducts(products, perPage, currentPage, sortBy));
  }, [products, perPage, currentPage, sortBy]);

  // UseEffect to set timeout for displaying "No products"
  useEffect(() => {
